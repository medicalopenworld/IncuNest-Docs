...
sidebar_position: 1
description: Documentação da API REST do IncuNest
...
A API REST do IncuNest permite controlar e monitorar o dispositivo remotamente.

**Base URL:** `http://<device_ip>/api`

**Formato de resposta:** JSON

**Autenticação:** Basic Auth (opcional, configurável)
...
Obteve o estado geral do dispositivo.
...
Obteve as leituras atuais de todos os sensores.
...
Obteve o histórico de leituras.

**Query Parameters:**
- `hours` (int): Horas de histórico (default: 24, max: 168)
- `interval` (int): Intervalo entre amostras em minutos (default: 5)
...
Obter a configuração actual de controlo.
...
Configura o controlo da temperatura.
...
**Body Parameters:**
- `setpoint` (float): Temperatura alvo (25.0 - 37.5)
- `mode` (string): "auto" | "manual"
- `manual_power` (int, opcional): Potência manual 0-100 se mode="manual"
...
Configura o controlo da humidade.
...
Obter o histórico de alarmes.

**Query Parameters:**
- `hours` (int): Horas de histórico (default: 24)
...
Obter a configuração actual do sistema.
...
::warning Precaução
Mudar os parâmetros PID pode afetar a estabilidade do controle. Faça pequenas mudanças e observe o comportamento.
::
...
Reestabelece a configuração da fábrica.
...
Obteve os logs do sistema.

**Query Parameters:**
- `lines` (int): Número de linhas (default: 100)
- `level` (string): Nível mínimo ("DEBUG", "INFO", "WARNING", "ERROR")
...
- Resposta 429 se o limite for superior
