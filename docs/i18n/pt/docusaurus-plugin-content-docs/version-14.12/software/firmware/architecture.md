---
id: architecture
title: Arquitetura do Firmware
sidebar_label: Arquitetura
sidebar_position: 1
description: Arquitetura do firmware de IncuNest
...
O firmware de IncuNest segue uma arquitetura modular baseada em camadas, facilitando a manutenção, testeing e extensibilidade.
...
|--------|-------------|---------------------|
| BOOT | Início do sistema | → INIT, ERROR |
| INIT | Inicialização de módulos | → SELFTEST |
| SELFTEST | Auto-diagnóstico | → STANDBY, ERROR |
| STANDBY | Esperando ativação | → PREHEATING |
| PREHEATING | Arrendamento de setpoint | → OPERATING, ALARM |
| OPERATING | Operação normal | → STANDBY, ALARM |
| ALARM | Condição de alarme | → OPERATING, EMERGENCY |
| EMERGENCY | Desligado de emergência | → STANDBY |
| ERROR | Erro crítico | → (requer serviço) |
