...
IncuNest firmware follows a modular, layer-based architecture, facilitating maintenance, testing and extensibility.
...
|--------|-------------|--------------|
| BOOT | System startup | → INIT, ERROR |
| INIT | Module initialization | → SELFTEST |
| SELFTEST | Self-diagnosis | → STANDBY, ERROR |
| STANDBY | Waiting for activation | → PREHEATING |
| PREHEATING | Warming up to setpoint | → OPERATING, ALARM |
| OPERATING | Normal operation | → STANDBY, ALARM |
| ALARM | Alarm condition | → OPERATING, EMERGENCY |
| EMERGENCY | Emergency shutdown | →STANDBY |
| ERROR | Critical error | → (service required) |
